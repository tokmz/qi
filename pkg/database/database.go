...
func New(cfg *Config) (*gorm.DB, error) {
	...
	if cfg.TracingEnabled {
		...
		if err := db.Use(NewTracingPlugin(opts...)); err != nil {
...
// 若 cfg.ZapLogger 不为 nil，使用 zap 适配器；否则退回 GORM 默认 logger。
func newLogger(cfg *Config) logger.Interface {
	if cfg.ZapLogger != nil {
		return newZapLogger(cfg.ZapLogger, cfg)
	}
	return logger.New(
		log.New(log.Writer(), "\r\n", log.LstdFlags),
		logger.Config{
...
func setupReadWriteSplit(db *gorm.DB, cfg *Config) error {
	...
	if len(rwCfg.Replicas) == 0 {
		return fmt.Errorf("read-write split enabled but no replicas provided")
	}

	// 构建从库 Dialector 列表
	replicas := make([]gorm.Dialector, 0, len(rwCfg.Replicas))
	for _, dsn := range rwCfg.Replicas {
