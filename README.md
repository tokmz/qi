...
// 国际化翻译（自动从上下文获取语言）
message := c.T("welcome")                    // 简单翻译
message := c.T("hello", "World")             // 带参数翻译
message := c.T("items_count", 5)             // 格式化翻译
```
...
func main() {
    // 初始化 i18n（可选，不初始化会使用默认配置）
    i18n.Init(i18n.Config{
        DefaultLanguage:    "en",
        FallbackLanguage:   "en",
        SupportedLanguages: []string{"en", "zh-CN", "ja"},
    })

    // 加载翻译文件
    i18n.LoadFromFile("zh-CN", "./locales/zh-CN.json")
    i18n.LoadFromFile("en", "./locales/en.json")

    // 或从 map 加载（自动添加语言到支持列表）
    i18n.LoadFromMap("zh-CN", map[string]string{
        "welcome":     "欢迎",
        "hello":       "你好，%s",
        "items_count": "共 %d 项",
    })
...
1. Query 参数 `lang`：`/api/users?lang=zh-CN`
2. Header `Accept-Language`：支持 quality value，选择 q 值最高的语言
3. Cookie `lang`
4. 默认语言
...
```go
r.Use(middleware.I18nWithConfig(middleware.I18nConfig{
    GetLanguage: func(c *qi.Context) string {
        // 从 JWT token 获取语言
        if lang := getLanguageFromToken(c); lang != "" {
            return lang
        }
        // 从用户配置获取
        if lang := getUserLanguage(c); lang != "" {
            return lang
        }
        return "en"
    },
...
```go
// 方式 1：使用 Context.T()（推荐）
r.GET("/hello", func(c *qi.Context) {
    message := c.T("welcome")
    greeting := c.T("hello", "World")
...
r.GET("/hello", func(c *qi.Context) {
    lang := qi.GetContextLanguage(c)
    message := i18n.T(lang, "welcome")
    c.Success(message)
})

// 方式 3：使用全局方法
message := i18n.T("zh-CN", "welcome")
greeting := i18n.T("en", "hello", "World")
```
...
// 使用快捷方法
logger.Info("用户登录", zap.String("username", "alice"))
logger.Error("数据库错误", zap.Error(err))
logger.Debug("调试信息", zap.Any("data", data))

// 使用 Logger 实例
log := logger.L()
log.Info("服务启动", zap.Int("port", 8080))

// 使用 SugaredLogger（更灵活但性能稍低）
sugar := logger.S()
sugar.Infof("用户 %s 登录成功", username)
sugar.Errorw("请求失败", "url", url, "error", err)
```
